...
class SpeedTrap:
    ...
    def trigger(self, frame: np.ndarray, detections: Detections):
        ...
        for xyxy, _, confidence, class_id, tracker_id in detections:
            ...
            triggers2 = [self.vector.is_in(point=anchor) for anchor in anchors]
            # detection is partially in and partially out
            if len(set(triggers)) == 2:
                continue

            tracker_state = triggers[0]
            print(f"1. {tracker_state}")
            # handle new detection
            if tracker_id not in self.tracker_state:
                self.tracker_state[tracker_id] = tracker_state
                continue
            print(f"2. {tracker_state}")
            # handle detection on the same side of the line
            if self.tracker_state.get(tracker_id) == tracker_state:
                continue
            print(f"3. {tracker_state}")
            self.tracker_state[tracker_id] = tracker_state
            print(f"4. {tracker_state}")
            if tracker_state:
                self.in_count += 1

            # else:
            #     self.out_count += 1
            #     # cv2.imwrite(f"salah_arah {tracker_id}.png", frame)
...
class SpeedAnnotate:
    ...
    def annotate(self, frame: np.ndarray, line_counter: SpeedTrap) -> np.ndarray:
        """
